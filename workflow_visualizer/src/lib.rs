...
use area::Area;
use render::{ExtractFns, RenderFns};
use crate::coord::DeviceContext;
use crate::gfx::{GfxOptions, GfxStack, GfxSurface};
use crate::job::{Job, TaskLabel};
use crate::render::{extract, Extract, invoke_extract, invoke_render, render, Render, RenderPhase};
        ...
        event_loop.run(move |event, event_loop_window_target, control_flow| match event {
            Event::NewEvents(start_cause) => match start_cause {
                StartCause::Init => {
                    ...
                    // invoke attach for core elements
