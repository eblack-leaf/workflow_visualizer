...
struct VertexInput {
    ...
    @location(4) instance_rasterization_key: vec3<u32>,

};
struct FragmentInput {
    @builtin(position) position: vec4<f32>,
    @location(0) instance_rasterization_index: vec3<u32>,
...
fn vertex_entry(
    ...
    let coordinates = vec4<f32>(
        ...
        instance_depth,
        1.0);
    return VertexOutput(viewport * coordinates, instance_rasterization_index, instance_color, instance_position);
}
@group(1)
@binding(0)
...
fn fragment_entry(
    ...
    let row_index = rasterization_px.y * fragment_input.instance_rasterization_key.y;
