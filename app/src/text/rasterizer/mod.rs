...
use crate::text::rasterizer::font::TextFont;
...
impl Rasterizer {
    ...
    pub fn rasterize(&mut self, font: TextFont, positioned_glyph: GlyphPosition) -> RasterizationKey {
        if let Some(rasterization) = self.rasterized_glyphs.get(&positioned_glyph.key) {
            return rasterization.1;
        }
        let glyph = font.font.rasterize(positioned_glyph.parent, positioned_glyph.key.px);
        ...
        self.rasterized_glyphs.insert(positioned_glyph.key, (glyph.clone(), rasterization_key));
