...
use mise_en_place::{
    ...
    DepthAdjust, Exit, GpuPosition, Icon, IconBundle, IconPlugin, IconSize, Idle,
    Job, Location, MessageHandler, MessageReceiverHandler, MouseAdapter, MouseButtonExpt, PartitionMetadata,
    Position, PositionAdjust, post_server, read_mesh, Text, TextBoundGuide, TextBundle,
...
fn update_text(
    ...
    for (entity, icon, click_state, position, area) in click_icon.iter() {
        if click_state.clicked() {
            ...
            println!("messages: {:?}", message_receiver_handler.messages());
        } else {
