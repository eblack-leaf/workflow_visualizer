...
pub fn prepare(canvas: Res<Canvas>, mut renderer: ResMut<TextRenderer>) {
    ...
    renderer.coordinator.start();
    renderer.coordinator.prepare::<Position>(&canvas);
    renderer.coordinator.prepare::<Area>(&canvas);
    renderer.coordinator.prepare::<Depth>(&canvas);
    renderer.coordinator.prepare::<Color>(&canvas);
    renderer
        .coordinator
        .prepare::<rasterization::Descriptor>(&canvas);
