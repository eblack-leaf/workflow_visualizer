...
impl ViewportOffset {
    pub(crate) fn new(position: Position<DeviceView>) -> Self {
        Self {
...
impl Viewport {
    pub(crate) fn new(device: &wgpu::Device, area: Area<DeviceView>) -> Self {
        ...
        let offset = ViewportOffset::new(Position::<DeviceView>::new(0.0, 0.0));
    ...
    pub(crate) fn as_section(&self) -> Section<DeviceView> {
        Section::new(
            Position::<DeviceView>::new(self.offset.offset[0], self.offset.offset[1]),
            self.cpu.area,
        )
    }
...
pub(crate) struct CpuViewport {
    pub(crate) area: Area<DeviceView>,
    pub(crate) depth: Depth,
    pub(crate) orthographic: nalgebra::Matrix4<f32>,
}

impl CpuViewport {
    pub(crate) fn new(area: Area<DeviceView>, depth: Depth) -> Self {
        Self {
            area,
            depth,
